const express = require("express");
const router = express.Router();
const portfolio = require("../models/protfolio");
const multer = require("multer");
const sharp = require("sharp");

const upload = multer({
  // dest: "photos",
  limits: {
    fileSize: 1048576,
  },
  fileFilter(req, file, cb) {
    if (!file.originalname.match(/.(jpg|jpeg|png)$/)) {
      return cb(new Error("please upload jpg,jpeg or png file"));
    }
    cb(undefined, true);
  },
});

router.post(
  "/portfolio/upload",
  upload.single("photo"),
  async (req, res) => {
    if(!req.file){
      return res.status(500).send({message:"please upload an image"})
    }
    const buffer = await sharp(req.file.buffer)
      .resize({ width: 2048, height: 1080 })
      .toBuffer();
    // const buffer=req.file.buffer
    const portfolio2 = new portfolio();
    portfolio2.photoBuffer = buffer;
    portfolio2.title=req.body.title;
    portfolio2.isBg = req.body.isBg;
    // await req.user.save();
    await portfolio2.save();
    res.send({status:"success"});
  },
  (error, req, res, next) => {
    res.status(400).send({ error: error.message });
  }
);

router.get("/portfolio/photo/:id", async (req, res) => {
  try {
    const [portfolio2] = await portfolio.find({ _id: req.params.id });
    if (!portfolio2 || !portfolio2.photoBuffer) {
      if(!req.file){
        return res.status(500).send({message:"no image found"})
      }
    }
    // res.set("Content-type", "image/png");
    res.send(portfolio2);
  } catch (error) {
    console.log(error)
    res.status(400).send(error);
  }
});

router.get("/portfolio/all", async (req, res) => {
  try {
    const response = await portfolio.find({});
    if (!response.length) {
      throw new Error();
    }
    // res.set("Content-type","image/png")
    res.send(response);
  } catch (error) {
    res.status(400).send(error);
  }
});

router.patch("/portfolio/update/:id",upload.single("photo"), async (req, res) => {
  try {
    
    const response = await portfolio.findById(req.params.id);
    // console.log(response)
    if (!response) {
      return res.status(500).send({message:"no image found"})
    }
    if(req.file){
      response.photoBuffer = await sharp(req.file.buffer)
      .resize({ width: 2048, height: 1080 })
      .toBuffer();
    }
    if(req.body.title){
      response.title=req.body.title
    }
    if(req.body.isBg){
        response.isBg=req.body.isBg
    }
    // res.set("Content-type","image/png")
    await response.save()
    res.send({status:"success"});
  } catch (error) {
    res.status(400).send(error);
  }
});

module.exports = router;
